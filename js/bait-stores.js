/**
 * Tiendas Bait-Yaavs — mismo módulo que Pospago, piel Bait.
 */
(function () {
  ...
  const DEFAULT_PDV_IMAGE = "assets/stores/fachadas/_fallback.jpg";
  ...
  function storesFor() {
    return window.BAIT_STORES || window.YAAVS_ATT_STORES || [];
  }
  ...
  function waUrl(store) {
    ...
    const msg = encodeURIComponent(
      `Hola, vi la sucursal ${store.name} en Bait-Yaavs y quiero información de planes Bait.`
    );
  ...
  function storeThumbHtml(store) {
    ...
    return `<div class="pospago-stores__map-pop"><img src="${src}" alt="Punto de venta ${name}" width="160" height="50" loading="lazy" decoding="async"><span>${name}</span></div>`;
  }
    ...
    listEl.innerHTML =
      shown
        .map((store, i) => {
          ...
          <button type="button" class="pospago-stores__card-media" data-store-photo="${store.id}" aria-label="Ver banner de ${escapeHtml(store.name)}">
            <img src="${escapeHtml(storeThumbSrc(store))}" alt="Banner ${escapeHtml(store.name)}" width="640" height="200" loading="lazy" decoding="async">
  ...
  function markerIcon(active) {
    return window.L.divIcon({
      ...
      html: `<span style="background:#ffd100"></span>`,
      iconSize: [22, 22],
      iconAnchor: [11, 11],
    });
  }
  ...
  function drawMap() {
    ...
    const list = filteredStores();
    if (!list.length) {
      setStatus("No se encontraron sucursales Bait.");
      if (map) {
        if (activePopup) {
          map.closePopup(activePopup);
          activePopup = null;
